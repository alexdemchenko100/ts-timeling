...
const RouteContainer = ({
    ...
    const locationKey = location.pathname
        .split("/")
        .filter(
            (x) => !!x && !/^\d+$/.test(x) && !x.startsWith("modal-") && !x.startsWith("section-")
        )
        .join("/");
    ...
    return (
        <ErrorBoundary>
            <main className={cx(`route-container route-container-${name}`, {})} ref={elem}>
                <Head>
                    {!!title && <title>{title}</title>}
                    <meta name="description" property="og:description" content={description} />
                </Head>
                <div className={cx(`content route route-${name}`, className)} {...otherProps}>
                    {children}
                </div>
            </main>
        </ErrorBoundary>
