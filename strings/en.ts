const strings = {
    months: "Months",
    days: "Days",
    hours: "Hours",
    minutes: "Minutes",
    seconds: "Seconds",
    accept_all: "Accept all",
    all_cookies: "All cookies",
    all_cookies_info: "Enable all cookies.",
    advertising_cookies: "Advertising cookies",
    advertising_cookies_info: "We use cookies to serve ads.",
    form_line_1: "Ready to meet?",
    form_line_2: "No time-wasting, just dating.",
    follow_us: "Follow us!",
    contact_us: "Contact us",
    cookies: "Cookies",
    cookie_settings: "Cookie settings",
    cookie_settings_text:
        "Both Timeling aswell as our partners use tools to understand our audience better and enhance the experience of the App. These tools allow us to provide features such as authentication.",
    cookies_consent: "By using our website, you agree to our cookie policy.",
    cookies_text:
        "We use cookies to help improve your experience on our website. This cookie policy is part of Timeling Limited's privacy policy, and covers the use of cookies between your device and our site.",
    coming_soon: "Coming soon",
    email: "Email",
    female: "Female",
    headline: "Date in real life",
    headline_sub: "Your calendar-based dating app for real life connections.",
    headline_sub2:
        "Sign up and get the exclusive chance to win a spot at our launch party at Sawan, Oslo.",
    headline_sub2_short: "Sign up and get the chance to join our launch party. ",
    dont_miss_out: "Don't miss out!",
    male: "Male",
    name: "Name",
    i_agree_to_the_0: "I agree to the {0}",
    by_click_0_you_agree_to_1: "By clicking {0} you are accepting our {1}",
    its_about_time: "It's about time.",
    notify_me: "Notify me",
    lets_meet_up: "Let's meet up",
    other: "Other",
    join_now: "Join now",
    join_the_waitlist: "Join waitlist",
    read_more: "Read more",
    save_preferences: "Save preferences",
    set_preferences: "Set preferences",
    scroll_dates_title: "Scroll dates,{0}not profiles",
    scroll_dates_content_short:
        "The dating app for, well, dates. Say goodbye to endless chatting and time-wasting.",
    scroll_dates_content_long:
        "Timeling is a calendar-based meet-up app that lets you set and go on dates. This saves you endless chatting, clashing expectations or ghosting – and helps you to actually meet new people, friends and maybe even the love of your life. So, stop swiping and start dating.",
    terms_of_service: "Terms of service",
    privacy_policy: "Privacy policy",
    press_realease: "Press release",
    time_we_met_title: "It's time we met",
    time_we_met_content_short:
        "Online dating, the Timeling way: Simply create a date, see who likes it and choose a match. Simple as that.",
    time_we_met_content_long:
        "A new kind of online dating: With Timeling, you can create a date, see who likes it and choose a match. Then you can chat or call to test the waters, confirm and meet up. No more wasting time, no more frustration. It’s a dating app with an active deadline, so to say.",
    ladies_first_title: "Say goodbye to bad dates",
    ladies_first_content_short:
        "We want you to enjoy your time with Timeling. That’s why we’ve developed a system that lets you confidentially share your experience with our admins.",
    ladies_first_content_long:
        "We want you to enjoy your time with Timeling. That’s why we've developed a system that lets you confidentially share your experience with our admins so that we can ensure a community that doesn't waste your time.	",
    "404_status": "Not found",
    thank_you_submission_recieved: "Thank you! Your submission has been received!",
    something_went_wrong: "Something went wrong 😢",
    unsubscribe: "Unsubscribe",
    _has_been_unsubscribed: "{0} has been unsubscribed",
    we_value_your_privacy: "We value your privacy",
    home_title: "Life is short, let's meet up.",
    home_des01start: "Timeling is a",
    home_des01middle: "calendar-based app",
    home_des01end:
        "that lets you create events, explore and meet like-minded people. Go on dates and meet new friends. ",
    home_des02start: "New friends and",
    home_des02middle: "maybe even the love",
    home_des02end: "of your life.",
    home_des03start: "No time-wasting,",
    home_des03middle: "just real connections. ",
    how_title: "How does it work?",
    how_title_dec:
        "We're changing the game, by putting the dates first. Instead of swiping through an endless stream of profiles, you can just create, explore, and go on dates.",
    work_title_01: "Set an event",
    work_title_02: "See who likes it",
    work_title_03: "Test the waters",
    work_title_04: "Let’s meet up",
    work_dec_01: " Set a day, time and activity for your next event.",
    work_dec_02: "See who is interested and test the waters.",
    work_dec_03: "Chat and see if you have a good vibe before you meet.",
    work_dec_04: "Confirm the event and get ready to meet.",
    m_work_title_01: "Create",
    m_work_dec_01:
        "Set a day, time and activity for your next event. See who is interested and test the waters.",
    m_work_title_02: "Explore",
    m_work_dec_02: "Find and filter events near you. Drop a like to show you’re interested!",
    m_work_title_03: "Confirm",
    m_work_dec_03: "Show interest, check your likes and make matches. It's time to meet!",
    about_us_title: "It’s time we met",
    about_us_title_desc1: "Imagine a world, where you sign up for an app to meet new people – and actually meet new people. A world, where endless swiping or chatting doesn’t exist, and you just go online to meet someone offline. A world, where matches, authenticity, and sincerity lead to something. And catfishing and ghosting don’t. ",
    about_us_title_desc2: "Imagine a world, where connections are to be experienced in real life, fun and full of purpose. A world, where you don’t feel like you’ve wasted your time – even if you are just trying to find someone to do exactly that with. This is the world of Timeling. A world, where you go to actually meet people. It’s about time. ",
    help_people_meet_title: "We help people meet in real life to make new connections.",
    help_people_meet_desc1: "Our core value proposition sums up what we do and our attitude to it.",
    help_people_meet_desc2: "We help busy people find time to meet in real life, by removing the typical frustrations of trying to meet someone online and empowering them to meet offline.",
    help_people_meet_desc3: "Your goal is to help people make more meaningful connections. We’re not about endless messaging or profile matching.",
    help_people_meet_desc4: "We’re about those time-stopping, real-life moments when two people just \"click\".",
    our_values_title: "Our values",
    our_values_desc: "We believe meeting new people should be smooth and sooner rather than later. Our values reflect and support this belief, keeping us focused on our cause – so keep them in mind at all times. They give our communications authority and consistency.",
    our_values_feature_1_title: "Effortless",
    our_values_feature_1_desc: "We don’t just want to save people time, we want to make meeting new people as easy as it gets.",
    our_values_feature_2_title: "Individual",
    our_values_feature_2_desc: "We’re passionate about individual choice. Whatever your preferences are, we say go for it.",
    our_values_feature_3_title: "Bold",
    our_values_feature_3_desc: "We’re fresh and straight to the point. Honesty is always the best policy.",
    our_values_feature_4_title: "Empowering",
    our_values_feature_4_desc: "Confidence gets you further. And we exist to give our Timelings more of it.",
};

export default strings;
